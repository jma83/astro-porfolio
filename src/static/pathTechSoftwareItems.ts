import type IconData from "../types/IconData.ts";

export const confluenceLogo: IconData = {
  src: "/images/path/tech/confluence_logo.avif",
  alt: "Confluence",
};

export const gitLogo: IconData = {
  src: "/images/path/tech/git_logo.avif",
  alt: "Git",
};

export const svnLogo: IconData = {
  src: "/images/path/tech/svn_logo.avif",
  alt: "SVN",
};

export const jiraLogo: IconData = {
  src: "/images/path/tech/jira_logo.avif",
  alt: "Jira",
};

export const togglLogo: IconData = {
  src: "/images/path/tech/toggl_logo.avif",
  alt: "Toggl",
};

export const trelloLogo: IconData = {
  src: "/images/path/tech/trello_logo.avif",
  alt: "Trello",
};

export const msProject: IconData = {
  src: "/images/path/tech/ms-project_logo.avif",
  alt: "Microsoft Project",
};

export const dockerLogo: IconData = {
  src: "/images/path/tech/docker_logo.avif",
  alt: "Docker",
};

export const firebaseLogo: IconData = {
  src: "/images/path/tech/firebase_logo.avif",
  alt: "Firebase",
};

export const androidLogo: IconData = {
  src: "/images/path/tech/android_logo.avif",
  alt: "Android",
};

export const iosLogo: IconData = {
  src: "/images/path/tech/ios_logo.avif",
  alt: "IOS",
};

export const getPathTechSoftwareItems = () => [
  jiraLogo,
  confluenceLogo,
  trelloLogo,
  togglLogo,
  msProject,
  gitLogo,
  svnLogo,
  dockerLogo,
  firebaseLogo,
];
